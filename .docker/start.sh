...
if [ "$role" = "django-http" ]; then
  ...
  if [ "True" = $SHOP_HOST4_REGISTER ]; then
    python3 manage.py create_host --owner=$DJANGO_OPERATOR_NAME --sitedomain="$SHOP_SITE_DOMAIN" --name="$SHOP_HOST4_NAME" --description="$SHOP_HOST4_DESCRIPTION" --ip=$SHOP_HOST4_IP --portranges=$SHOP_HOST4_PORT_RANGES --rangetype=$SHOP_HOST4_PORT_RANGE_TYPE --isenabled=$SHOP_HOST4_IS_ENABLED --isalive=$SHOP_HOST4_IS_ALIVE --istestnet=$SHOP_HOST4_IS_TESTNET --offerstorbridges=$SHOP_HOST4_OFFERS_TOR_BRIDGES --torbridgeduration=$SHOP_HOST4_TOR_BRIDGE_DURATION --torbridgepriceinitial=$SHOP_HOST4_TOR_BRIDGE_PRICE_INITIAL --torbridgepriceextension=$SHOP_HOST4_TOR_BRIDGE_PRICE_EXTENSION --offersrsshtunnels=$SHOP_HOST4_OFFERS_RSSH_TUNNELS --rsshtunnelprice=$SHOP_HOST4_RSSH_TUNNEL_PRICE --tos="$SHOP_HOST4_TERMS_OF_SERVICE" --tosurl="$SHOP_HOST4_TERMS_OF_SERVICE_URL" --cistatus=$SHOP_HOST4_CI_STATUS --cidate="$SHOP_HOST4_CI_DATE" --cimessage="$SHOP_HOST4_CI_MESSAGE"
  fi
